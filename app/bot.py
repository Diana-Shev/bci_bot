...
2. Результат должен быть СТРОГО в JSON формате. JSON должен содержать следующие ключи:
   ...
   - "report_text": строка, содержащая подробный аналитический отчет со всеми разделами (Обзор, Оптимальные показатели и т.д.), отформатированный как текст.
3. Внутри поля "report_text" отчет должен содержать разделы:
   ...
   8. Алгоритм отдыха.
   9. Рекомендуемое расписание в виде таблицы (как часть текстового отчета).
   10. Выводы.
4. Используй реальные значения из данных (приводи даты, время и показатели).
5. Отчёт внутри "report_text" должен быть подробным, с пояснениями и ссылками на научные эффекты (например, влияние альфа-ритма на внимание).
...
```json
{
  ...
  "report_text": "Подробный аналитический отчет со всеми разделами..."
}
```
...
"""
    ...
    report_text = data.get("report_text", "") or "" # Извлекаем report_text
    ...
    # Изменяем сообщение для пользователя, чтобы включить report_text
    msg = ""
    if report_text:
        msg += report_text # Весь отчет теперь в этом поле
    else:
